...
function VideoTab({ trailers, movie, onDelete, onRefresh }) {
  const navigate = useNavigate();
  const location = useLocation();

  useEffect(() => {
    // Refresh trailers when returning from add-trailer page
    if (onRefresh) {
      onRefresh();
    }
  }, [location]);
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        ...
        <button
          onClick={() => navigate(`/movie/${movie.movieId}/add-trailer`)}
