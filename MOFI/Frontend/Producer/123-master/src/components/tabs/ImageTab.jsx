function ImageTab({ images, movieId, onDelete, onShowModal, onImageClick, hasImagePermission }) {
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-bold text-white">Images Gallery</h2>
        {hasImagePermission && onShowModal && (
          <button
      ...
      {images && images.length > 0 ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {images.map((image) => (
            <div 
              key={image.id} 
              className="group relative bg-gray-800 rounded-xl overflow-hidden border border-gray-700 hover:border-amber-500 transition-all shadow-lg hover:shadow-2xl hover:shadow-amber-500/20 cursor-pointer"
              ...
              <div className="p-4">
                ...
                <div className="mt-2 flex items-center justify-between">
                  <span className="text-xs text-gray-400">
                    {hasImagePermission ? "Can Edit" : "View Only"}
                  </span>
